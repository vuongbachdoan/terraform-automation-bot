...
async function ensureTerraformInstalled() {
  try {
    await execAsync("terraform -version");
    console.log(chalk.green(`${icons.success} Terraform is already installed.`));
  } catch (err) {
    console.log(chalk.yellow(`${icons.warn} Terraform not found. Installing...`));
    const spinner = ora("Installing Terraform...").start();
    ...
    try {
      ...
      spinner.succeed(`${icons.success} Terraform installed successfully.`);
    } catch (installErr) {
      spinner.fail(`${icons.warn} Terraform installation failed.`);
...
(async () => {
  printGreeting();
  await ensureTerraformInstalled();

  const { action } = await prompts({
    type: "select",
    name: "action",
    message: chalk.hex("#80EF80")("What would you like to do?"),
    choices: [
      { title: "Analyze & Refactor Terraform", value: "refactor" },
  ...
  if (action === "refactor") {
    const { dir } = await prompts({
      type: "text",
      name: "dir",
      message: chalk.hex("#80EF80")("Enter directory to analyze:"),
      initial: ".",
    });

    await analyzeAndRefactor(path.resolve(dir));
  } else {
